...
<div class="schedule-form">
    ...
    <form action="{{ route('superadmin.schedules.update', $schedule->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <select id="field_id" name="field_id" required>
                ...
                @foreach($fields as $field)
                    <option value="{{ $field->id }}" {{ old('field_id', $schedule->field_id) == $field->id ? 'selected' : '' }}>{{ $field->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="time" id="open_time" name="open_time" value="{{ old('open_time', $schedule->open_time) }}" required>
        </div>

        <div class="form-group">
            <label for="close_time">Close Time</label>
            <input type="time" id="close_time" name="close_time" value="{{ old('close_time', $schedule->close_time) }}" required>
        </div>

        <div class="form-group">
            <label for="price_per_hour">Price per Hour (Rp)</label>
            <input type="number" id="price_per_hour" name="price_per_hour" value="{{ old('price_per_hour', $schedule->price_per_hour) }}" step="0.01" required>
        </div>
